...
def verts2voxel(verts, voxel_size):
    orig_voxel = np.zeros(voxel_size, dtype=bool)
    verts = verts.astype(int)
    for index in range(verts.shape[0]):
        orig_voxel[verts[index, 0], verts[index, 1], verts[index, 2]] = True
    return orig_voxel
...
def normalize_verts(verts):
    ...
    x_scale = 128.0 / (max_x - min_x)
    y_scale = 128.0 / (max_y - min_y)
    z_scale = 128.0 / (max_z - min_z)
