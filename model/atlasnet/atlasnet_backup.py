"""
   The script is used for atlasnet v2 backup modified from atlasnet 
"""
...
from model.atlasnet.model_blocks import Mapping2Dto3D, Identity
from model.atlasnet.template import get_template
...
class Atlasnet(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.nb_pts_in_primitive = opt.pts_num // opt.nb_primitives
    ...
    def forward(self, latent_vector, train=True):
        ...
        # Deform each patch
        output_points = torch.cat([self.decoder[i](input_points[i], latent_vector.unsqueeze(2)).transpose(1, 2).unsqueeze(1) for i in
                                   range(0, self.opt.nb_primitives)], dim=1)

        # Return the deformed pointcloud
        return output_points.contiguous(), latent_vector  # batch, nb_prim, num_point, 3
