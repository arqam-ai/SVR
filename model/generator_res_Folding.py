...
class GeneratorRes(nn.Module):
	def __init__(self, width, codelength, input_dim = 2):
		...
		#self.layer7 = PointwiseMLP([width, width], doLastRelu=False)
		#self.layer8 = PointwiseMLP([width, width], doLastRelu=False)
		#self.layer9 = PointwiseMLP([width, width], doLastRelu=False)
		#self.layer10 = PointwiseMLP([width, width], doLastRelu=False)
		...
		#self.bn7 = nn.BatchNorm1d(width)
		#self.bn8 = nn.BatchNorm1d(width)
		#self.bn9 = nn.BatchNorm1d(width)
		#self.bn10 = nn.BatchNorm1d(width)
        
		self.layer11 = PointwiseMLP([width, 64], doLastRelu=True)        
		self.layer12 = PointwiseMLP([64, 3], doLastRelu=False)
        
	def forward(self, X):
		X = self.input_layer(X)

		#X = self.bn0(X.permute(0, 2, 1)).permute(0, 2, 1)
		...
		#Y = self.layer7(X)
		#Y = self.bn7(Y.permute(0, 2, 1)).permute(0, 2, 1)
		#X = self.layer8(Y + X)
		#X = self.bn8(X.permute(0, 2, 1)).permute(0, 2, 1)
        
		#Y = self.layer9(X)
		#Y = self.bn9(Y.permute(0, 2, 1)).permute(0, 2, 1)
		#X = self.layer10(Y + X)
		#X = self.bn10(X.permute(0, 2, 1)).permute(0, 2, 1)
...
class GeneratorVanilla(nn.Module):
	...
	def __init__(self, grid_dims, resgen_width, resgen_depth, 
		         resgen_codelength, class_num, MLP_doLastRelu=False, read_view = False, folding_twice = False):
	...
	def forward(self, X, view=None):
		...
		if self.read_view:
			img_feat = img_feat + self.read_view_branch(view)
        
		img_feat = img_feat.unsqueeze(1)                # B* 1 * 512
		#img_feat = self.fc_linear(img_feat)                 
