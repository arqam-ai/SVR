...
DECODER_OPTION={"ResnetFC": ResnetBlockFC,         # no batch norm
                "ResnetFCBN": ResnetBlockFCBN,     # batch norm 
                "ResnetFCGN": ResnetBlockFCGN,     # group norm 
                "FC": BlockFC,                     # no batch norm
                "FCBN": BlockFCBN,                 # batch norm 
                "FCGN": BlockFCGN}                 # group norm  
...
class GeneratorResFC(nn.Module):
    def __init__(self, hidden_neurons, bottleneck_size, num_layers, activation, input_dim = 2, remove_all_batchNorms=False, decoderBlock=None):
        ...
        if decoderBlock == "ResnetFCBN" or decoderBlock == "FCBN":
            self.bn0 = nn.BatchNorm1d(self.hidden_neurons)
            self.bn1 = nn.BatchNorm1d(64)
        elif decoderBlock == "ResnetFCGN" or decoderBlock == "FCGN":
            self.bn0 = nn.GroupNorm(4, self.hidden_neurons)
            self.bn1 = nn.GroupNorm(4, 64)

        self.ResBlock_list = nn.ModuleList(
        [DECODER_OPTION[decoderBlock](size_in=hidden_neurons, size_out=hidden_neurons) for _ in range(0, int(self.num_layers/2))])

    def forward(self, X):
        if self.decoderBlock == "ResnetFC" or self.decoderBlock == "FC":
