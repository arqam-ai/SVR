'''
    This script is used to show the class distribution of random sampling 
...
def main(args):
    ...
    train_num, train_class_dic, train_class_list, train_fileid_list = class_counter(data_basedir=args.data_basedir,
                                                                                    ...
                                                                                    class_path=args.class_path,
                                                                                    split_name='train')
    for exp_order in range(1, 11):
        sample_index = np.load('faster_ss_data/testset_ptcloud_random2000_index_{}.npy'.format(exp_order))
        #print(max(sample_index))
        #print(min(sample_index))
        sample_class_list = [test_class_list[sample_index[i]] for i in range(sample_index.shape[0]) ]
        sample_type = {}
        for idx, class_name in enumerate(sample_class_list):
            if class_name in sample_type:
                sample_type[class_name] += 1
            else:
                sample_type[class_name] = 1
        #for ix, shape in enumerate(sample_type.keys()):
        #    print("{}, {}".format(shape, sample_type[shape]))
        print('table = {}'.format(sample_type['table']))
