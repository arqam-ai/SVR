...
from compute_testset_distance_matrix import Block_compute_ptcloud_dismatrix, Block_compute_codeword_dismatrix
...
def codeword_distance(args):
    codeword = np.load(args.data_path)
    codeword = np.squeeze(codeword, axis = 1)

    N = codeword.shape[0]
    # initialize distance matrix
    D = torch.zeros([N, N])
    distance_matrix = nn.L1Loss().to(args.device)
    codeword = torch.from_numpy(codeword).to(args.device)
...
def main(args):
    ...
    ptcloud_set = np.load(args.data_path)
    ...
    '''
    # Load the training dataset
    data_set = np.load(args.data_path)
    # Convert numpy array to torch tensor
    data_set = torch.from_numpy(data_set).to(args.device)
    # define the distance between point cloud 
    if args.distance_matrix == 'CD':
        criterion = ChamferDistance().to(args.device)
    elif args.distance_matrix == 'L1':
    ...
    Path(partial_DM_path).mkdir(parents=True, exist_ok=True)
    # compute the block distance matrix and save 
    train_pt_matrix = Block_compute_codeword_dismatrix(X1=data_set, X2=data_set,
                                                     distance_metric=criterion,
                                                     start_index=args.start_index,
                                                     end_index=args.end_index,
                                                     title='trainset_mat_{}_{}_{}.npy'.format(
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data-path", type=str,
                        ...
                        help=' ')
    parser.add_argument('--codeword-path', type=str,
                        default="../experiment/resnet_twofold/traincodeword_resnet_nolabel_twofold/final_vis/codeword_trainset.npy", 
                        help='')
